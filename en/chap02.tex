...
\textbf{Results evaluation}\label{sec:results-assessment}
...
\footnotetext{The SQL-only implementation of both evaluation metrics is available in the GitHub repository of this thesis.} %TODO SQL LINK
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        We use a preselected threshold distance to determine the mergeability of the nodes.
        \item We use the clustering results to merge all nodes belonging to the same cluseters.
    \end{enumerate}
...
For a list of nodes, we assume the algorithm to return a ``cluster list'' - a list of the node sets representing different clusters.

\subsubsection{Results evaluation}

As with the \hyperref[sec:results-assessment]{naïve approach evaluation}, we evaluate the performance of the hierarchical clustering method on the set of the internal authors,
as we have the ground truth for the identities of these authors.

The \textit{merging algorithm to multi-class classifier} mapping works in this case as well, with slight differences.
For the purpose of the evaluation, we again reformulate the identity inference as a classification problem:

For a node $n$ without an identifier and a (generated) cluster $C_{\text{n}}$ containing $n$ (and some other nodes with the same normalized name),
